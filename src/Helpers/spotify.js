...
const redirectUri = "http://localhost:3000/copy";
...
export const authLink =  authEndpoint + '?client_id=' + clientId + '&redirect_uri=' + redirectUri + '&scope=' + scopes.join("%20") + '&response_type=token&show_dialog=true&state=' + makeid(16);
...
export const getPlaylistTracks = async (token, playlistId, next) => {
  let url = next ? next : `https://api.spotify.com/v1/playlists/${playlistId}/tracks`;
  let response = await fetch(url, {
    method: 'GET',
    headers: new Headers({
    Accept: "application/json",
